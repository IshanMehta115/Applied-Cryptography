/*
Name : Ishan Mehta
Roll : 2019309

Implementing AES with all key variants

*/

#include <iostream>
#include <fstream>
#include <string>
using namespace std;
string get_data(string filename)
{
    fstream newfile;
    string data = "";
    newfile.open(filename, ios::in);
    if (newfile.is_open())
    { // checking whether the file is open
        getline(newfile, data);
        newfile.close();
    }
    else
    {
        cout << "Cannot open file: " << filename << "\n";
    }
    return data;
}
void display(const unsigned char * c)
{
    for(int i=0;i<16;i++)
    {
        cout<<hex<<(int)c[i]<<" ";
    }
    cout<<"\n";
}
class AES
{
private:
    int l, nk, nr, cipher_key_length, expanded_key_length;
    static const int state_length = 16;
    unsigned char *key, *expanded_key, *plaintext, *ciphertext, *state;

    const unsigned char rcon[256] = {
        };

    const unsigned char s_box[256] =
        {
            };

    const unsigned char inverted_s_box[256] =
        {
            };

    const unsigned char mul02[256] =
        {
            };

    const unsigned char mul03[256] =
        {
            };

    const unsigned char mul09[256] =
        {
            };

    const unsigned char mul11[256] =
        {
            };

    const unsigned char mul13[256] =
        {
            };

    const unsigned char mul14[256] =
        {
            };

    void key_expansion_core(unsigned char *w, unsigned short i)
    {
        unsigned char t = w[0];
        w[0] = s_box[w[1]];
        w[1] = s_box[w[2]];
        w[2] = s_box[w[3]];
        w[3] = s_box[t];
        w[0] = w[0] ^ rcon[i];
    }

    void SubWord(unsigned char *w)
    {
        w[0] = s_box[w[0]];
        w[1] = s_box[w[1]];
        w[2] = s_box[w[2]];
        w[3] = s_box[w[3]];
    }

    void key_expansion()
    {
        const int rows = 4;
        for (int i = 0; i < nk * 4; i++) expanded_key[i] = key[i];

        unsigned short byGen = nk * rows;
        unsigned short rconIdx = 1;
        unsigned char temp[rows];

        while (byGen < (nr + 1) * state_length)
        {
            for (unsigned short i = 0; i < rows; i++)
            {
                temp[i] = expanded_key[byGen - rows + i];
            }
            if (byGen % (nk * rows) == 0) key_expansion_core(temp, rconIdx++);
            else if ((nk > 6) && (byGen % (nk * rows) == (4 * rows))) SubWord(temp);
            for (int i = 0; i < rows; i++)
            {
                expanded_key[byGen] = expanded_key[byGen - nk * rows] ^ temp[i];
                byGen++;
            }
        }
    }

public:
    void set_security_parameter(int par)
    {
        l = par;
        nk = l / 32;
        cipher_key_length = nk * 4;
        if (4 == nk) nr = 10;
        if (6 == nk) nr = 12;
        if (8 == nk) nr = 14;
        expanded_key_length = (nr + 1) * state_length;
    }
    void set_key(string k)
    {
        int nn = k.size();
        key = new unsigned char[nn];
        for(int i=0;i<nn;i++) key[i] = k[i];
        expanded_key = new unsigned char[expanded_key_length];
        key_expansion();
        cout<<"Expanded key ->\n";
        int index = 0;
        for(int i=0;i<=nr;i++)
        {
            cout<<dec<<"Round "<<i<<" ->\n";
            for(int j=0;j<16;j++)
            {
                cout<<hex<<(int)expanded_key[index++]<<" ";
            }
            cout<<"\n";
        }
        cout<<"\n\n";
    }
    void add_round_key(int start)
    {
        for (int i = 0; i < state_length; i++) state[i] ^= expanded_key[i + start];
    }
    void sub_bytes()
    {
        for (unsigned short i = 0; i < state_length; i++) state[i] = s_box[state[i]];
    }
    void shift_rows()
    {
        unsigned char tmp_state[state_length];
        for(int i=0;i<state_length;i++) tmp_state[i] = state[(i + (4 * (i % 4))) % 16];
        for (int i = 0; i < state_length; i++) state[i] = tmp_state[i];
    }
    void mix_columns()
    {
        unsigned char tmp_state[state_length];
        tmp_state[0] = mul02[state[0]] ^ mul03[state[1]] ^ state[2] ^ state[3];
        tmp_state[1] = state[0] ^ mul02[state[1]] ^ mul03[state[2]] ^ state[3];
        tmp_state[2] = state[0] ^ state[1] ^ mul02[state[2]] ^ mul03[state[3]];
        tmp_state[3] = mul03[state[0]] ^ state[1] ^ state[2] ^ mul02[state[3]];


        tmp_state[4] = mul02[state[4]] ^ mul03[state[5]] ^ state[6] ^ state[7];
        tmp_state[5] = state[4] ^ mul02[state[5]] ^ mul03[state[6]] ^ state[7];
        tmp_state[6] = state[4] ^ state[5] ^ mul02[state[6]] ^ mul03[state[7]];
        tmp_state[7] = mul03[state[4]] ^ state[5] ^ state[6] ^ mul02[state[7]];


        tmp_state[8] = mul02[state[8]] ^ mul03[state[9]] ^ state[10] ^ state[11];
        tmp_state[9] = state[8] ^ mul02[state[9]] ^ mul03[state[10]] ^ state[11];
        tmp_state[10] = state[8] ^ state[9] ^ mul02[state[10]] ^ mul03[state[11]];
        tmp_state[11] = mul03[state[8]] ^ state[9] ^ state[10] ^ mul02[state[11]];


        tmp_state[12] = mul02[state[12]] ^ mul03[state[13]] ^ state[14] ^ state[15];
        tmp_state[13] = state[12] ^ mul02[state[13]] ^ mul03[state[14]] ^ state[15];
        tmp_state[14] = state[12] ^ state[13] ^ mul02[state[14]] ^ mul03[state[15]];
        tmp_state[15] = mul03[state[12]] ^ state[13] ^ state[14] ^ mul02[state[15]];
        for (unsigned short i = 0; i < state_length; i++) state[i] = tmp_state[i];

    }
    void aes_encrypt()
    {
        state = new unsigned char[state_length];
        for (unsigned short i = 0; i < state_length; i++) state[i] = plaintext[i];

        add_round_key(0);
        for (unsigned short i = 0; i < nr; i++)
        {   
            sub_bytes();
            shift_rows();
            if (i != (nr - 1)) mix_columns();
            add_round_key(state_length * (i + 1));
        }
        for (int i = 0; i < state_length; i++) ciphertext[i] = state[i];

    }
    void encrypt(unsigned char *pt, unsigned char *buffer)
    {
        plaintext = new unsigned char[16];
        ciphertext = new unsigned char[16];
        for(int i=0;i<16;i++) plaintext[i] = pt[i];
        aes_encrypt();
        for(int i=0;i<state_length;i++) buffer[i] = ciphertext[i];
    }
    void inv_shift_rows()
    {
        unsigned char tmp_state[state_length];
        for (int i = 0; i < state_length; i++) tmp_state[i] = state[((i - (4 * (i % 4)))%16 + 16) % 16];
        for (unsigned short i = 0; i < state_length; i++) state[i] = tmp_state[i];
    }
    void inv_sub_bytes()
    {
        for (int i = 0; i < state_length; i++) state[i] = inverted_s_box[state[i]];
    }
    void inv_mix_columns()
    {
        unsigned char tmp_state[state_length];
        tmp_state[0] = mul14[state[0]] ^ mul11[state[1]] ^ mul13[state[2]] ^ mul09[state[3]];
        tmp_state[1] = mul09[state[0]] ^ mul14[state[1]] ^ mul11[state[2]] ^ mul13[state[3]];
        tmp_state[2] = mul13[state[0]] ^ mul09[state[1]] ^ mul14[state[2]] ^ mul11[state[3]];
        tmp_state[3] = mul11[state[0]] ^ mul13[state[1]] ^ mul09[state[2]] ^ mul14[state[3]];

        tmp_state[4] = mul14[state[4]] ^ mul11[state[5]] ^ mul13[state[6]] ^ mul09[state[7]];
        tmp_state[5] = mul09[state[4]] ^ mul14[state[5]] ^ mul11[state[6]] ^ mul13[state[7]];
        tmp_state[6] = mul13[state[4]] ^ mul09[state[5]] ^ mul14[state[6]] ^ mul11[state[7]];
        tmp_state[7] = mul11[state[4]] ^ mul13[state[5]] ^ mul09[state[6]] ^ mul14[state[7]];

        tmp_state[8] = mul14[state[8]] ^ mul11[state[9]] ^ mul13[state[10]] ^ mul09[state[11]];
        tmp_state[9] = mul09[state[8]] ^ mul14[state[9]] ^ mul11[state[10]] ^ mul13[state[11]];
        tmp_state[10] = mul13[state[8]] ^ mul09[state[9]] ^ mul14[state[10]] ^ mul11[state[11]];
        tmp_state[11] = mul11[state[8]] ^ mul13[state[9]] ^ mul09[state[10]] ^ mul14[state[11]];

        tmp_state[12] = mul14[state[12]] ^ mul11[state[13]] ^ mul13[state[14]] ^ mul09[state[15]];
        tmp_state[13] = mul09[state[12]] ^ mul14[state[13]] ^ mul11[state[14]] ^ mul13[state[15]];
        tmp_state[14] = mul13[state[12]] ^ mul09[state[13]] ^ mul14[state[14]] ^ mul11[state[15]];
        tmp_state[15] = mul11[state[12]] ^ mul13[state[13]] ^ mul09[state[14]] ^ mul14[state[15]];
        for (unsigned short i = 0; i < state_length; i++) state[i] = tmp_state[i];
    }
    void aes_decrypt()
    {
        state = new unsigned char[state_length];
        for (unsigned short i = 0; i < state_length; i++) state[i] = ciphertext[i];

        add_round_key(state_length * nr);

        for (int i = 0; i < nr; i++)
        {
            inv_shift_rows();
            inv_sub_bytes();
            add_round_key(state_length * (nr - i - 1));
            if (i != (nr - 1))
            {
                inv_mix_columns();
            }
        }
        for (unsigned short i = 0; i < state_length; i++) plaintext[i] = state[i];

    }

    void decrypt(unsigned char *ct, unsigned char *buffer)
    {
        plaintext = new unsigned char[16];
        ciphertext = new unsigned char[16];
        for(int i=0;i<16;i++) ciphertext[i] = ct[i];
        aes_decrypt();
        for(int i=0;i<state_length;i++) buffer[i] = plaintext[i];
    }
};
int main()
{
    cout << "Enter security parameter l (128,192,256)\n";
    int l;
    cin >> l;
    if (!(128 == l || 192 == l || 256 == l))
    {
        cout << "Security parameter error\n";
        return 0;
    }
    string plaintext = get_data("Q1_plaintext.txt");
    if ("" == plaintext)
    {
        cout << "Plaintext error\n";
        return 0;
    }
    string key = get_data("Q1_key.txt");
    if ("" == key)
    {
        cout << "Key error\n";
        return 0;
    }

    unsigned char *buffer, *ct, *pt, *dt;
    AES *a = new AES();
    a->set_security_parameter(l);
    a->set_key(key);
    pt = new unsigned char[16];
    buffer = new unsigned char[16];
    for(int i=0;i<16;i++) pt[i]  = plaintext[i];
    cout << "Plaintext ->\n";
    display(pt);
    a->encrypt(pt, buffer);
    ct = new unsigned char[16];
    for(int i=0;i<16;i++) ct[i] = buffer[i];
    cout << "Ciphertext ->\n";
    display(ct);
    a->decrypt(ct, buffer);
    dt = new unsigned char[16];
    for (int i = 0; i < 16; i++) dt[i] = buffer[i];
    cout << "Decrypted text ->\n";
    display(dt);
    return 0;
}